...
class DiscordBooster:
    ...
    async def get_boost_slots(self):
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                # Filter slots that are not currently used or can be reassigned
                return response.json()
            return []
...
async def boost_with_token(token, invite_code):
    ...
    if not available_slots:
        return {"success": False, "message": f"No available boosts on account {user['username']}"}

    # 4. Apply Boosts
    success, detail = await booster.apply_boosts(guild_id, available_slots)
    if success:
        return {"success": True, "message": f"Successfully boosted with {user['username']}! ({len(available_slots)} boosts applied)"}
    else:
